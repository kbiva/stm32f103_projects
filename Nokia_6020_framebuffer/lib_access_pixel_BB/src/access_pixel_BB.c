...
void rasterLine_pixel_BB( int x0, int y0, int x1, int y1, int *rl )
{
  // Calculate deltas
  int dx = abs(x1 - x0);
  int dy = abs(y1 - y0);
             ...
             unsigned char _c)
{
  ...
  rasterLine_pixel_BB( _x0, _y0, _x1, _y1, rl );
  rasterLine_pixel_BB( _x1, _y1, _x2, _y2, rl );
  rasterLine_pixel_BB( _x2, _y2, _x3, _y3, rl );
  rasterLine_pixel_BB( _x3, _y3, _x0, _y0, rl );
...
void drawPolygons_pixel_BB( float *tp, int *pp )
{
  ...
  for (p=0;p<nbFaces;++p)
  {
    ...
    drawPolygon_pixel_BB( pp[2*p0+0], pp[2*p0+1],
          pp[2*p1+0], pp[2*p1+1],
          pp[2*p2+0], pp[2*p2+1],
          pp[2*p3+0], pp[2*p3+1],
          faces_color[pi]);
  ...
  for(i=0;i<200;i++)
  {
    ...
    for (x=0;x<MAX_Y;x++) {
      ...
      for (y=0;y<MAX_X;++y) {
        ...
        PCF8833_Command_Bitbang(PCF8833_PASET);
        PCF8833_Data_Bitbang(x);
        PCF8833_Data_Bitbang(x);
        PCF8833_Command_Bitbang(PCF8833_CASET);
        PCF8833_Data_Bitbang(y);
        PCF8833_Data_Bitbang(y);
        PCF8833_Command_Bitbang(PCF8833_RAMWR);
        if((xx<16 && yy<16) || (xx>16 && yy>16)) {
          PCF8833_Data_Bitbang(WHITE);
        } else {
          PCF8833_Data_Bitbang(BLACK);
        }
      }
    }

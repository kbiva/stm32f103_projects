/*
 *  access_framebuffer_BB.c
 *
 *  library for accessing Nokia 6020 LCD
 *  using framebuffer and bit banging
 *
...
extern uint8_t framebuffer1[];  // 19602 bytes
...
void drawFrame_framebuffer_BB(void) {
  ...
  PCF8833_Data_Bitbang(START_X);
  PCF8833_Data_Bitbang(END_X);
  PCF8833_Command_Bitbang(PCF8833_CASET);
  PCF8833_Data_Bitbang(START_Y);
  PCF8833_Data_Bitbang(END_Y);
  PCF8833_Command_Bitbang(PCF8833_RAMWR);

  for(i=0;i<MAX_X*MAX_Y;i++){
    PCF8833_Data_Bitbang(framebuffer1[i]);
  }
}

void rasterLine_framebuffer_BB( int x0, int y0, int x1, int y1, int *rl )
{
  // Calculate deltas
  int dx = abs(x1 - x0);
  int dy = abs(y1 - y0);
             ...
             unsigned char _c)
{
  ...
  rasterLine_framebuffer_BB( _x0, _y0, _x1, _y1, rl );
  rasterLine_framebuffer_BB( _x1, _y1, _x2, _y2, rl );
  rasterLine_framebuffer_BB( _x2, _y2, _x3, _y3, rl );
  rasterLine_framebuffer_BB( _x3, _y3, _x0, _y0, rl );
...
void drawPolygons_framebuffer_BB( float *tp, int *pp )
{
  ...
  for (p=0;p<nbFaces;++p)
  {
    ...
    drawPolygon_framebuffer_BB( pp[2*p0+0], pp[2*p0+1],
          pp[2*p1+0], pp[2*p1+1],
          pp[2*p2+0], pp[2*p2+1],
          pp[2*p3+0], pp[2*p3+1],
          faces_color[pi]);
...
void display_rotating_cube_checkboard_framebuffer_BB(void) {
  ...
  for(i=0;i<200;i++)
  {
      ...
      for (y=0;y<MAX_X;++y) {
        yy = (50+y+yo)% 32;
         if((xx<16 && yy<16) || (xx>16 && yy>16)) {
           framebuffer1[(x*MAX_Y+y)]=WHITE;
         }
         else {
           framebuffer1[(x*MAX_Y+y)]=BLACK;
         }
      }
    }
    ...
    drawPolygons_framebuffer_BB( tp, pp );

    drawFrame_framebuffer_BB();

/*
 *  access_framebuffer_SPI.c
 *
 *  library for accessing Nokia 6020 LCD
 *  using framebuffer and SPI
 *
...
void drawFrame_framebuffer_SPI(void) {
  ...
  for(i=0;i<MAX_X*MAX_Y*9/8;i++){
    while (!(SPI1->SR & SPI_I2S_FLAG_TXE)){};
    SPI1->DR = framebuffer1[i];
  }
}

void rotating_cube_checkboard_ClearBuf_DMA_framebuffer_SPI(void) {

  DMA1->IFCR |= ((uint32_t)(DMA_ISR_GIF6 | DMA_ISR_TCIF6 | DMA_ISR_HTIF6 | DMA_ISR_TEIF6));

  DMA1_Channel6->CNDTR = (MAX_X*MAX_Y*9/8)/4;
...
void rotating_cube_checkboard_initDMA_framebuffer_SPI(void) {
...
void rasterLine_framebuffer_SPI( int x0, int y0, int x1, int y1, int *rl )
{
  // Calculate deltas
  int dx = abs(x1 - x0);
  int dy = abs(y1 - y0);
             ...
             unsigned char _c)
{
  ...
  rasterLine_framebuffer_SPI( _x0, _y0, _x1, _y1, rl );
  rasterLine_framebuffer_SPI( _x1, _y1, _x2, _y2, rl );
  rasterLine_framebuffer_SPI( _x2, _y2, _x3, _y3, rl );
  rasterLine_framebuffer_SPI( _x3, _y3, _x0, _y0, rl );
...
void drawPolygons_framebuffer_SPI( float *tp, int *pp )
{
  ...
  for (p=0;p<nbFaces;++p)
  {
    ...
    drawPolygon_framebuffer_SPI( pp[2*p0+0], pp[2*p0+1],
          pp[2*p1+0], pp[2*p1+1],
          pp[2*p2+0], pp[2*p2+1],
          pp[2*p3+0], pp[2*p3+1],
          faces_color[pi]);
...
void display_rotating_cube_checkboard_framebuffer_SPI(void) {
  ...
  // DMA initialization
  rotating_cube_checkboard_initDMA_framebuffer_SPI();

  for(i=0;i<200;i++)
  {
    rotating_cube_checkboard_ClearBuf_DMA_framebuffer_SPI();
    ...
    drawPolygons_framebuffer_SPI( tp, pp );

    drawFrame_framebuffer_SPI();
